import { supabase, getUserByAuthId, recordFontSelection, updateFontSelectionCart, createPurchaseOrder, addPurchaseItem, createUserLicense, updatePurchaseOrderStatus } from './supabaseClient';
...
export class FontDownloadService {
  ...
  async generateDownloadLink(userId, userLicenseId, fontStyleId, fileFormat, clientInfo = {}) {
    try {
      // Verify user has license for this font
      const hasLicense = await this.verifyUserLicense(userId, userLicenseId, fontStyleId);
      if (!hasLicense) {
        return { success: false, error: 'License not found or expired' };
      }
      ...
      const { data, error } = await createFontDownload(downloadData);
      ...
      const downloadUrl = `${this.downloadBaseUrl}/${data.download_token}`;
  ...
  async verifyUserLicense(userId, userLicenseId, fontStyleId) {
    try {
      const { data, error } = await supabase
        .from('user_licenses')
