...
export function processLongDescription(longDescription) {
  ...
  const processParagraph = (paragraph) => {
    ...
    const isProductionCredit =
      ...
      /^(Printing:|Edition:|Published by|Production:|Photography by:|Dimensions:)/i.test(
        processedText,
      ) ||
      // Physical items and format details
      /^(T-Shirt:|Size:|Format:)/i.test(processedText) ||
      // Credit and support acknowledgments
      /^(Made with support from|With thanks to:|Purchase from:)/i.test(
        processedText,
      ) ||
      // Venue and location statements (specific format only)
      /^(Williams Lounge, Wellcome Collection|Goldsmiths Allotment|PAKT Gallery|Pump House Gallery|Perdu Literary Foundation|Goethe Institute|San Serriffe|London Design Festival|Moravian Gallery|Brno Biennale)/i.test(
        processedText,
      ) ||
      ...
      /^('Display Unit' was originally constructed|Produced while in residence at:)/i.test(
        processedText,
      ) ||
      ...
      (/^[^\.]+, (UK|US|NL|CZ|JP|NO|Estonia)\.$/i.test(processedText) &&
        processedText.length < 60);
    ...
    processedText = processedText.replace(
      /\b(CMYK|RGB|B&W|NO\.|UK|US|NL|CZ|JP)\b/g,
    ...
    processedText = processedText.replace(
      /(Printing|Edition|Production|Published by|Photography by|Dimensions|T-Shirt|Size|Format|Purchase from):/g,
      "$1: ",
    );
