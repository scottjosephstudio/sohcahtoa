...
const HomeIconContainer = styled(motion.div)`
  ...
  visibility: ${({
    ...
    return $isNavigating || $shouldHideForTransition || !$ShouldShow
      ? "hidden"
      : "visible";
  }};
  transition: ${({ $isTypefacesPage }) =>
    $isTypefacesPage
      ? "visibility 0.5s ease, left 0.8s ease"
      : "opacity 0.5s ease, visibility 0.5s ease, left 0.8s ease"};
