...
export const validateField = (type, value) => {
  switch (type) {
    ...
    case 'postcode':
      ...
      if (trimmedPostcode.toUpperCase().startsWith('UK') && !ukPostcodeRegex.test(trimmedPostcode)) {
        return 'Invalid UK postcode format';
      }
      if (trimmedPostcode.toUpperCase().startsWith('US') && !usZipCodeRegex.test(trimmedPostcode)) {
...
export const getCustomStripeError = (error) => {
  ...
  const errorMessages = {
    'Your card\'s expiry year is in the past': 'Your card\'s expiry year is in the past',
    'Your card\'s expiry month is in the past': 'Your card\'s expiry month is in the past',
