...
const CartPanelContent = ({ isOpen, onClose, onNavigateHome, setIsLoggedIn }) => {
        ...
        >
          <CartHeader 
            ...
            onStageChange={(changes) => {
              if (changes.showPaymentForm !== undefined) setShowPaymentForm(changes.showPaymentForm);
              if (changes.showUsageSelection !== undefined) setShowUsageSelection(changes.showUsageSelection);
              if (changes.showRegistration !== undefined) setShowRegistration(changes.showRegistration);
              if (changes.selectedUsage !== undefined) setSelectedUsage(changes.selectedUsage);
              if (changes.eulaAccepted !== undefined) setEulaAccepted(changes.eulaAccepted);
              if (changes.isAuthenticatedAndPending !== undefined) setIsAuthenticatedAndPending(changes.isAuthenticatedAndPending);
              if (changes.summaryModifiedAfterTab !== undefined) setSummaryModifiedAfterTab(changes.summaryModifiedAfterTab);
              ...
              if (changes.didReturnToStageOne !== undefined) setDidReturnToStageOne(changes.didReturnToStageOne);
            ...
            >
              ...
              >
                {(processPayment) => (
                  ...
                  >
                  <motion.div key="payment-section" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                      <PaymentSection 
                  ...
                  <motion.div key="payment-summary" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                        <CartSummary
                          weightOption={weightOption}
...
<CartContent 
  ...
  $hasSelections={!!(weightOption || selectedPackage || (customizing && getActiveLicenses().length > 0))}
>
            <motion.div key="main-content" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
            ...
            <motion.div key="main-summary" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                <CartSummary
                  weightOption={weightOption}
                  selectedPackage={selectedPackage}
                  customizing={customizing}
...
export const CartPanel = ({ isOpen, onClose, onNavigateHome, setIsLoggedIn }) => {
return (
<CartProvider onClose={onClose} isOpen={isOpen} setIsLoggedIn={setIsLoggedIn} onNavigateHome={onNavigateHome}>
<CartPanelContent 
isOpen={isOpen} 
onClose={onClose}
onNavigateHome={onNavigateHome}
