...
import { isUserAuthenticated, loginUser, getCartState, saveCartState } from './authUtils';
...
export const CartProvider = ({ children, onClose, isOpen, setIsLoggedIn }) => {
  ...
  useEffect(() => {
    if (!preventAutoScroll && (showRegistration || showUsageSelection || showPaymentForm)) {
      scrollToTop();
    }
  }, [showRegistration, showUsageSelection, showPaymentForm, preventAutoScroll]);
  ...
  useEffect(() => {
    ...
    if (!preventAutoScroll && shouldScrollToBottom && bottomPadding > 0 && cartPanelRef.current && window.matchMedia('(max-width: 768px)').matches) {
  ...
  const updateBottomPadding = (isMobile) => {
    if (isMobile) {
      ...
      const hasSelections = weightOption && (selectedPackage || (customizing && hasLicenseSelected()));
      ...
      if (totalSectionRef.current) {
        ...
        requestAnimationFrame(() => {
          setTimeout(() => {
            ...
            const currentHasSelections = weightOption && (selectedPackage || (customizing && hasLicenseSelected()));
            ...
            if (totalSectionRef.current) {
              const fixedTotalHeight = totalSectionRef.current.getBoundingClientRect().height || 0;
              if (fixedTotalHeight > 0) {
                setBottomPadding(fixedTotalHeight + 24);
              } else {
                // Fallback - try again with longer delay
                setTimeout(() => {
                  const finalHasSelections = weightOption && (selectedPackage || (customizing && hasLicenseSelected()));
                  if (finalHasSelections && totalSectionRef.current) {
                    const retryHeight = totalSectionRef.current.getBoundingClientRect().height || 0;
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/create-payment-intent', {
        ...
        body: JSON.stringify({
          amount: selectedPackage ? packages[selectedPackage].price : calculateCustomTotal(),
          currency: 'gbp',
          forceNew: true,
  ...
  const handleProceed = () => {
    ...
    if ((showRegistration || showUsageSelection || showPaymentForm) && !isCheckoutDisabled) {
      handleFormProceed();
    }
...
const isPaymentFormValid = () => {
  ...
  const hasCartSelections = () => {
    return weightOption && (selectedPackage || (customizing && hasLicenseSelected()));
  };
 
  const hasLicenseSelected = () => {
    return selectedPackage || 
      (customizing && (customPrintLicense || customWebLicense || customAppLicense || customSocialLicense));
  };
  ...
  const getCurrentStage = () => {
    ...
    if (showUsageSelection || (showRegistration && !isRegistrationComplete)) return 2;
    return 1;
  };
  ...
  const handleRemoveLicense = (type) => {
    ...
    switch(type) {
      case 'print': setCustomPrintLicense(null); break;
      case 'web': setCustomWebLicense(null); break;
      case 'app': setCustomAppLicense(null); break;
      case 'social': setCustomSocialLicense(null); break;
    }
        ...
        if (isContinueClicked) {
          setTimeout(() => {
            if (cartPanelRef.current) {
              ...
              const customLicenseSection = cartPanelRef.current.querySelector('[data-custom-license]');
              if (customLicenseSection) {
                const yOffset = -101;
                const y = customLicenseSection.getBoundingClientRect().top + cartPanelRef.current.scrollTop + yOffset;
                cartPanelRef.current.scrollTo({
                  top: y,
                  behavior: 'smooth'
                });
    ...
    try {
      ...
      const { error: paymentError, paymentIntent } = await stripe.confirmPayment({
        ...
        confirmParams: {
          payment_method_data: {
            billing_details: {
              name: savedRegistrationData?.firstName + ' ' + savedRegistrationData?.surname,
      ...
      inputTimeoutRef.current = setTimeout(() => {
        ...
        if (isMobileView) {
          if (formDividerRef.current && cartPanelRef.current) {
            const formDividerRect = formDividerRef.current.getBoundingClientRect();
            const panelRect = cartPanelRef.current.getBoundingClientRect();
            
            const loginFormOffset = showLoginForm ? 658 : 54;
            
            const targetScroll = cartPanelRef.current.scrollTop + 
              (formDividerRect.bottom - panelRect.bottom) + 
              formDividerRect.height - loginFormOffset;
    ...
    if (ref.current && cartPanelRef.current) {
      ...
      if (!isMobileView) {  // Desktop view
        ...
        if (buttonBottom > panelBottom) {
          ...
          const targetScroll = cartPanelRef.current.scrollTop + (buttonBottom - panelBottom) + loginOffset;
    ...
    if (cartPanelRef.current && formDividerRef.current) {
      ...
      setTimeout(() => {
        if (!isMobileView) {
          ...
          const scrollTarget = Math.min(formDividerRef.current.offsetTop + 300, fullHeight);
  ...
  const handleBackToLogin = () => {
    ...
    setTimeout(() => {
      if (cartPanelRef.current && formDividerRef.current) {
        ...
        if (!isMobileView) {
          const scrollTarget = Math.min(formDividerRef.current.offsetTop + 300, cartPanelRef.current.scrollHeight);
  ...
  const loadSavedCartState = (savedCart) => {
    ...
    if (!showRegistration && !showUsageSelection) {
      ...
      const hasSelections = savedCart.weightOption && (
    ...
    const handlePackageSelect = (pkg) => {
      ...
      if (isDeselecting) {
        ...
        if (window.matchMedia('(max-width: 768px)').matches && totalSectionRef.current) {
          const totalHeight = totalSectionRef.current.getBoundingClientRect().height || 0;
      ...
      if (cartPanelRef.current) {
        ...
        setTimeout(() => {
          ...
          if (isMobileView) {
            ...
            const summarySection = cartPanelRef.current.querySelector('[data-mobile-summary]');
            if (summarySection) {
              const yOffset = -24; // Small offset from top
              const y = summarySection.getBoundingClientRect().top + cartPanelRef.current.scrollTop + yOffset;
              cartPanelRef.current.scrollTo({
                top: Math.max(0, y),
                behavior: 'smooth'
              });
    ...
    const handleLicenseSelect = (type, value) => {
      ...
      const currentValue = (() => {
        switch(type) {
          case 'print': return customPrintLicense;
          case 'web': return customWebLicense;
          case 'app': return customAppLicense;
          case 'social': return customSocialLicense;
          default: return null;
        }
      })();
      ...
      switch(type) {
        case 'print': setCustomPrintLicense(newValue); break;
        case 'web': setCustomWebLicense(newValue); break;
        case 'app': setCustomAppLicense(newValue); break;
        case 'social': setCustomSocialLicense(newValue); break;
      }
      ...
      const previousActiveCount = [customPrintLicense, customWebLicense, customAppLicense, customSocialLicense].filter(license => license !== null).length;
      ...
      if (newActiveCount === 0) {
        ...
        if (window.matchMedia('(max-width: 768px)').matches) {
          ...
          const isFirstLicense = previousActiveCount === 0 && newActiveCount === 1;
    ...
    const handleUsageComplete = () => {
      if ((selectedUsage === 'personal' || (selectedUsage === 'client' && isClientDataValid()))) {
        setIsRegistrationComplete(true);
        setIsAuthenticatedAndPending(false);
