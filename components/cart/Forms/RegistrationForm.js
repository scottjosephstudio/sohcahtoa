...
export const Registration = ({
  ...
  const handleBlur = (fieldName, value) => {
    ...
    if (!value?.trim()) {
      error = fieldName.replace(/([A-Z])/g, ' $1')
      ...
      switch (fieldName) {
        case 'country': {
          ...
          if (error) {
            ...
            const postcodeError = validatePostcode(registrationData.postcode, value);
            setFieldErrors(prev => ({
              ...prev,
  ...
  return (
    <StepContainer $isRegistration>
      {fields.map((section, sectionIndex) => (
        ...
        >
          <OptionHeader $isBillingHeader={section.section === "Billing Address"}>
            {section.section}
          </OptionHeader>
                  ...
                  {field.name === 'password' ? (
                    <PasswordContainer>
                      <FormInput
                        ...
                        placeholder={showFieldErrors[field.name] ? fieldErrors[field.name] : ''}
                        required
                      />
                      <TogglePasswordButton
                        type="button"
      ...
      <RegisterButton
        ...
        initial="initial"
        animate={(!isRegistrationFormValid || Object.values(fieldErrors).some(error => error)) ? "disabled" : "enabled"}
        whileHover={(!isRegistrationFormValid || Object.values(fieldErrors).some(error => error)) ? {} : "hover"}
        whileTap={(!isRegistrationFormValid || Object.values(fieldErrors).some(error => error)) ? {} : "hover"}
        disabled={!isRegistrationFormValid || Object.values(fieldErrors).some(error => error)}
      >
        Register
      </RegisterButton>
