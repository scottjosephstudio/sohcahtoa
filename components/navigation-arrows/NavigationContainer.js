...
const NavigationContainer = styled(motion.div)`
  ...
  visibility: ${({ $isNavigating, $ShouldShow }) => ($isNavigating || !$ShouldShow ? 'hidden' : 'visible')};
  transform: translateX(${({ $isSpecialPage, $animatedSlide }) => 
    $isSpecialPage && $animatedSlide ? '-150%' : '0'
  });
  transition: ${({ $isTypefacesPage }) => 
    $isTypefacesPage ? 'visibility 0.5s ease, transform 0.8s ease' : 'opacity 0.5s ease, visibility 0.5s ease, transform 0.8s ease'
  };
  -webkit-transition: ${({ $isTypefacesPage }) => 
    $isTypefacesPage ? 'visibility 0.5s ease, transform 0.8s ease' : 'opacity 0.5s ease, visibility 0.5s ease, transform 0.8s ease'
  };
  mix-blend-mode: ${({ $isTypefacesPage }) => $isTypefacesPage ? 'normal' : 'difference'};
  display: ${({ $isIDPath, $forceShowOnIDPath }) => ($isIDPath && !$forceShowOnIDPath) ? 'none' : 'flex'};
