...
export default function NavigationArrows({
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 2; i++) {
      const prevIndex =
        (currentIndex - i + allProjects.length) % allProjects.length;
    ...
    const newLinksToPrefetch = linksToPrefetch.filter(
      (link) => !prefetchedLinks.includes(link),
    );
    ...
    return (
      <NavigationContainer
        ...
        {...(isTypefacesPage
          ...
          : {})}
      >
        <NavigationButtonStatic whileHover="hover" initial="initial">
          <PrevTooltip
            className="tooltip"
            $isTypefacesPage={isTypefacesPage}
