...
export default function TransitionWrapper({ children, onTransitionComplete, disableTransition = false }) {
 ...
 useEffect(() => {
   ...
   const handleClick = (e) => {
     ...
     if (shouldExcludeHomeIcon || shouldExcludeNavArrows || shouldExcludeSpinner) {
       return;
     }
     
     // Skip menu links only when currently on typefaces page (they handle their own navigation)
     if (pathname === '/Typefaces' && e.target.closest('nav, .Overlay, .MenuContainer, .MenuPill, .DropdownContent')) {
       return;
     }
