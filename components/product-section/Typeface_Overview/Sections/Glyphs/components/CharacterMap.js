...
const GlyphCanvas = ({ char, font, selected, hoveredGlyph, onSelect, onHover, onHoverEnd }) => {
  ...
  const drawGlyph = useCallback(() => {
    ...
    if (isHovered || selected) {
      console.log(`Rendering ${char} as ${isHovered ? 'hovered' : 'selected'} - should be white`);
    }
    ...
    return Object.entries(font.characterSets).map(([setName, setInfo]) => {
      ...
      if (setInfo.ranges) {
        ...
        setInfo.ranges.forEach(range => {
          glyphs = glyphs.concat(generateCharacterRange(range.start, range.end));
        });
      } else {
        // Handle single range
