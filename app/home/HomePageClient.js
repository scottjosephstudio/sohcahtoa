...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const adjustedInitialCount =
      Math.floor(initialCount / columnsPerRow) * columnsPerRow;
    ...
    const initialImages = shuffled.slice(
      0,
      Math.min(finalInitialCount, shuffled.length),
    );
              ...
              visibleImages.map((image, index) => (
                <motion.div key={image.id} variants={itemVariants}>
                  <ProjectTile href={`/Projects/${targetProjectId}`}>
                    ...
                    />
                    <ProjectCaption className="caption">
